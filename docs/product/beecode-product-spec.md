...
不同产品端根据自身运行环境获得不同的工具能力。CLI 和桌面端可以使用经过权限控制的本机文件、Shell、Git 等能力；Web 和移动端不提供依赖用户本机环境的工具，并向用户明确说明不可用原因。与本机无关的工具可以在五端统一提供。
...
| 用户电脑的文件、Shell、Git | 按能力与权限开放 | 按能力与权限开放 | 不可用 | 不可用 | 不可用 |
